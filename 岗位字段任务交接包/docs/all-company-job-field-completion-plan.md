...
更新时间：2026-09-01  
状态：Wells Fargo 已完成 Workday 生产字段回填和全公司严格回归；下一家进入 Accenture；按来源族逐批执行，不做全站批量回填
...
生产美国服务器已重新运行 `audit:company-source-matrix`。当前 `collector_feed` 有 75 家活跃公司、57,389 条在招岗位；下表是后续实施的唯一排期依据，而不是按公司名称随意处理。
...
| Workday | Wells Fargo 1,076、Accenture 2,328、Citigroup 2,158 | 已识别来源族；Wells Fargo 及此前已列公司的生产字段回填已完成 | 下一家公司处理 Accenture；仍须独立确认官方 host、tenant、site、稳定 ID 和真实字段样本 |
...
| --- | --- | --- | --- |
| 1 | BCG / Phenom | 已完成生产字段回填、20 条公网详情抽样、误关闭保护和三次独立同步观察 | 已通过；进入常规来源观察 |
...
| 4 | Houlihan Lokey 至 Adobe / Workday | 以低岗位量公司建立 Workday 协议层，按公司保存 tenant / host 配置 | 已完成；继续观察生产队列 |
...
**扩展门槛：** 每家公司通过完整生产验收后才进入下一家；当前已完成 17 家 Workday 公司，Wells Fargo 已完成官方来源、20 条真实样本、dry-run、写入和回归验收。

**当前进度（2026-09-01）：** Houlihan Lokey、Bain Capital、Apollo Global Management、The Carlyle Group、Adobe、Brookfield、PIMCO、Blackstone、Barclays、Ares Management、Intel、State Street、Fidelity Investments、Bank of America、Vanguard、NVIDIA、Wells Fargo 已完成首轮生产回填与全公司回归验收。Wells Fargo 最终在招 1,076 条，完整 dry-run 507 条候选、正式更新 228 条、失败 0，末端 `62650`，最终候选为 0；字段证据审计和公网/API/官方详情抽样通过，strict 回归为 `company_count: 75`、`regressions: []`。Workday CXS 详情没有提供工作方式、经验年限和截止日期时继续保持空；每家公司必须使用自己的 tenant、host、site、requisition ID 和真实样本，不能复制另一家公司的 URL 或字段假设。下一家公司进入 Accenture，各公司的完成数字和命令见 `岗位字段任务交接包/docs/job-field-task-handoff.md`。
...
| R4 | 批次 3 的前九家 | Workday 连接器、Houlihan Lokey、Bain Capital、Apollo、Carlyle、Adobe、Brookfield、PIMCO、Blackstone、Ares Management 已完成首轮生产回填与回归验收 | 已完成公司继续观察；下一家公司按 20 条样本门槛进入剩余 Workday |
| R5 | 剩余 Workday、Apple | 从 Intel 起按岗位量递增的分段回填记录；Apple 独立适配器 | 每个公司均遵守 20 条样本后才进入后续分段写入 |
