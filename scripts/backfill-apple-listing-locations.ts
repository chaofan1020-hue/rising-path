...
function isAppleJobUrl(value: string | null): boolean {
  if (!value) return false;
  try {
    const url = new URL(value);
    return url.protocol === 'https:'
      && url.hostname.toLowerCase() === 'jobs.apple.com'
      && /\/details\/\d+/i.test(url.pathname);
  } catch {
    return false;
  }
}
...
function locationEvidence(row: JobRow, verifiedAt: string, evidenceUrl: string): Record<string, unknown> {
  ...
  return {
    ...
    source_type: typeof current.source_type === 'string' ? current.source_type : 'apple',
...
async function main(): Promise<void> {
  ...
  if (write && process.env.APPLE_LISTING_LOCATION_WRITE_ENABLED !== 'true') {
    throw new Error('写入默认关闭；请同时设置 APPLE_LISTING_LOCATION_WRITE_ENABLED=true 和 --write');
  }
  ...
  if (write) {
    for (let index = 0; index < candidates.length; index += WRITE_BATCH) {
      ...
      for (const row of batch) {
        ...
        if (lastError) throw new Error(`更新 Apple 岗位 ${row.id} 地点证据失败: ${lastError}`);
