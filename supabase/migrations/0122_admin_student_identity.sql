...
create or replace function public.admin_student_public_code(p_user_id uuid)
returns text
language sql
immutable
as $$
  select 'LV-' || upper(substr(replace(p_user_id::text, '-', ''), 1, 8));
$$;

create or replace function public.get_admin_student_directory_v2(
...
create or replace function public.get_admin_student_directory_v2_count(
  p_search text default null,
  p_email_user_id uuid default null
)
returns bigint
...
revoke all on function public.admin_student_public_code(uuid) from public, anon, authenticated;
revoke all on function public.get_admin_student_directory_v2(text, text, integer, integer, uuid) from public, anon, authenticated;
revoke all on function public.get_admin_student_directory_v2_count(text, uuid) from public, anon, authenticated;
grant execute on function public.admin_student_public_code(uuid) to service_role;
grant execute on function public.get_admin_student_directory_v2(text, text, integer, integer, uuid) to service_role;
grant execute on function public.get_admin_student_directory_v2_count(text, uuid) to service_role;
