...
-- Full-library retrieval without ranking every match in the database. Each
-- high-signal resume term uses the GIN FTS index to contribute a small,
-- bounded candidate set; candidates are deduplicated and only that set is
-- ranked with the combined resume query. This keeps unfiltered AI matching
-- under the Supabase statement timeout while still searching every job.
create or replace function public.search_ai_match_candidates_v5(
