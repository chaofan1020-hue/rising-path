...
规划器只选择意图、维度、场景、角度和相关简历证据，不生成候选人可见话术。服务端使用 `session_seed + sessionId + revision + answer hash` 签名，聊天接口只接受签名规划，客户端不能伪造岗位或公司策略。正常语音作答不需要候选人点击确认：ASR `final` 到达后等待 450ms 合并相邻片段并自动提交；只有发送失败时才显示“重试发送 / 丢弃识别稿”的紧急处理。
