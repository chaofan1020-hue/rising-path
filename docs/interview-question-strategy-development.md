...
当前会话消息已经进入 LLM 上下文时，不要再把同一批完整问题复制到 noRepeatNote 中。当前实现使用会话级 `context_digest` 和 `facts_ledger`：前者保留岗位、DNA、候选人经历和技能的结构化摘要，后者记录已覆盖维度、候选人已作出的事实主张与未验证缺口。每轮按当前意图从原始简历证据中召回少量相关片段；完整原文留在简历版本和 turn 表中，不无限注入，也不因缩短 prompt 而丢失。
