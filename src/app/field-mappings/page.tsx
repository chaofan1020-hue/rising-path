...
import { ArrowRight, BriefcaseBusiness, Check, ChevronRight, ClipboardList, FilePenLine, Loader2, Puzzle, Save, Sparkles } from 'lucide-react';
...
const copy: Record<Locale, Record<string, string>> = {
  'zh-CN': {
    eyebrow: 'Liorvix 网申中心', title: '网申中心', subtitle: '从选择岗位、准备档案到追踪进度，所有网申步骤集中在一个工作区。最终提交始终由你完成。', prepTitle: '准备一次有把握的申请', prepDescription: '先选岗位和简历，再进入岗位详情确认匹配度并打开招聘官网。', profileSettings: '档案设置', loading: '加载中…', start: '开始网申', records: '申请记录', profile: '求职档案', back: '返回岗位',
  ...
  'zh-TW': {
    eyebrow: 'Liorvix 網申中心', title: '網申中心', subtitle: '從選擇職位、準備檔案到追蹤進度，所有網申步驟集中在一個工作區。最終提交始終由你完成。', prepTitle: '準備一次有把握的申請', prepDescription: '先選職位和履歷，再進入職位詳情確認匹配度並開啟招聘官網。', profileSettings: '檔案設定', loading: '載入中…', start: '開始網申', records: '申請記錄', profile: '求職檔案', back: '返回職位',
  ...
  en: {
    eyebrow: 'Liorvix application center', title: 'Application center', subtitle: 'Choose a role, prepare your profile, and track every application from one workspace. You always submit yourself.', prepTitle: 'Prepare a focused application', prepDescription: 'Choose a role and resume, then review the match and open the employer site.', profileSettings: 'Profile settings', loading: 'Loading…', start: 'Start application', records: 'Applications', profile: 'Profile', back: 'Back to jobs',
...
export type ApplicationCenterTab = 'start' | 'profile' | 'records';

export function AutoApplicationContent({ initialTab = 'start' }: { initialTab?: ApplicationCenterTab }) {
  const { locale } = useLanguage(); const c = copy[locale];
  const [activeTab, setActiveTab] = useState<ApplicationCenterTab>(initialTab); const [profile, setProfile] = useState<ApplicationProfile | null>(null); const [source, setSource] = useState<Record<string, ProfileSource>>({}); const [profileVersion, setProfileVersion] = useState(0); const [profileLoading, setProfileLoading] = useState(true); const { loading: resumesLoading, error: resumesError, availability, reload: reloadResumes } = useResumeAvailability<ResumeOption>(); const resumes = availability.eligible; const [jobs, setJobs] = useState<JobOption[]>([]); const [jobSearch, setJobSearch] = useState(''); const [selectedJobId, setSelectedJobId] = useState<number | null>(null); const [selectedResumeId, setSelectedResumeId] = useState<number | null>(null); const [saving, setSaving] = useState(false); const [saveDone, setSaveDone] = useState(false); const [aiFilling, setAiFilling] = useState(false); const [aiFillJob, setAiFillJob] = useState<AiFillJob | null>(null); const [starting, setStarting] = useState(false);
  useEffect(() => {
    const tab = new URLSearchParams(window.location.search).get('tab');
    const resumeParam = Number(new URLSearchParams(window.location.search).get('resumeId'));
    if (Number.isInteger(resumeParam) && resumeParam > 0) {
      setSelectedResumeId(resumeParam);
      writeActiveResumeId(resumeParam);
    }
    if (tab === 'profile') setActiveTab('profile');
    else if (tab === 'applications' || tab === 'records') setActiveTab('records');
    else if (tab === 'start') setActiveTab('start');
  }, []);
  const loadProfile = useCallback(async () => { setProfileLoading(true); try { const query = selectedResumeId ? `?resumeId=${encodeURIComponent(String(selectedResumeId))}` : ''; const response = await apiFetch(`/api/application-profile${query}`); const data = await response.json(); if (!response.ok) throw new Error(data.error || 'profile'); setProfile(data.profile); setSource(data.fieldStats || data.source || {}); setProfileVersion(typeof data.version === 'number' ? data.version : 0); const resolvedResumeId = typeof data.resumeId === 'number' ? data.resumeId : null; if (resolvedResumeId && resolvedResumeId !== selectedResumeId) { setSelectedResumeId(resolvedResumeId); writeActiveResumeId(resolvedResumeId); } const nextJob = (data.aiJob || null) as AiFillJob | null; setAiFillJob(nextJob); setAiFilling(nextJob?.status === 'pending' || nextJob?.status === 'running'); } catch (error) { console.error('Failed to load application profile:', error); } finally { setProfileLoading(false); } }, [selectedResumeId]);
  useEffect(() => { void loadProfile(); }, [loadProfile]);
  const aiFillJobId = aiFillJob?.id;
  const aiFillJobStatus = aiFillJob?.status;
  useEffect(() => {
    if (!aiFillJobId || (aiFillJobStatus !== 'pending' && aiFillJobStatus !== 'running')) return;
    let stopped = false;
    let timer: ReturnType<typeof setTimeout> | null = null;
    const poll = async () => {
      try {
        const response = await apiFetch(`/api/application-profile/ai-fill?jobId=${aiFillJobId}`);
  ...
  }, [aiFillJobId, aiFillJobStatus, loadProfile]);
  useEffect(() => { apiFetch('/api/jobs?summary=1&limit=100').then((response) => response.ok ? response.json() : { jobs: [] }).then((data) => setJobs(data.jobs || [])).catch(() => setJobs([])); }, []);
  useEffect(() => { if (resumesLoading) return; const stored = readActiveResumeId(); const next = pickEligibleResumeId(resumes, selectedResumeId || stored); if (next !== selectedResumeId) { setSelectedResumeId(next); if (next) writeActiveResumeId(next); } }, [resumes, resumesLoading, selectedResumeId]);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto max-w-6xl px-4 pb-16 pt-24 md:px-6 md:pt-28">
        <header className="mb-8 flex items-start gap-3">
          <span className="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-red-600 text-white shadow-sm shadow-red-600/20"><ClipboardList className="h-5 w-5" /></span>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-red-600 dark:text-red-400">{c.eyebrow}</p>
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as ApplicationCenterTab)} className="gap-6">
          <TabsList className="h-11 w-full justify-start gap-1 rounded-lg border bg-card p-1 sm:w-fit">
            ...
            <TabsTrigger className="px-4" value="records">{c.records}</TabsTrigger>
          </TabsList>
            ...
            <section className="overflow-hidden rounded-lg border bg-card">
              ...
              <div className="grid divide-y md:grid-cols-[1fr_1fr] md:divide-x md:divide-y-0">
                ...
                <WorkflowStep number="2" title={c.stepResume} hint={c.resumeHint} complete={Boolean(selectedResumeId)}>
                  {resumes.length ? <Select value={selectedResumeId ? String(selectedResumeId) : undefined} onValueChange={(value) => { const id = Number(value); setSelectedResumeId(id); writeActiveResumeId(id); setAiFillJob(null); setAiFilling(false); void apiFetch('/api/resume/active', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ resumeId: id }) }); }}><SelectTrigger><SelectValue placeholder={c.selectResume} /></SelectTrigger><SelectContent>{resumes.map((resume) => <SelectItem key={resume.id} value={String(resume.id)}>{resume.file_name}</SelectItem>)}</SelectContent></Select> : <ResumeAvailabilityHint status={availability.status} loading={resumesLoading} error={resumesError ? c.noResume : ''} onRetry={() => void reloadResumes(true)} />}
                </WorkflowStep>
              </div>
          ...
          <TabsContent value="records" className="mt-0"><ApplicationList /></TabsContent>
        </Tabs>
      </main>
    </div>
  );
...
export default function FieldMappingsPage() {
  ...
  useEffect(() => { router.replace('/applications?tab=profile'); }, [router]);
  return null;
}
