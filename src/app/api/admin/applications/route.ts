...
function applyApplicationFilters<T extends {
  ...
  const clauses = [`notes.ilike.*${search}*`];
  if (/^\d+$/.test(search)) {
    clauses.push(`id.eq.${search}`, `job_id.eq.${search}`, `resume_id.eq.${search}`);
  }
...
export async function GET(request: NextRequest) {
  ...
  if (status && !isApplicationStatus(status)) {
    return NextResponse.json(
      { data: null, error: { code: 'INVALID_STATUS', message: '网申状态无效' } },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    let query = client
      .from('applications')
      .select(
        'id, job_id, resume_id, user_id, status, notes, submitted_at, created_at, updated_at, jobs(title, company, region, direction), resumes(file_name)',
        { count: 'exact' },
      )
    ...
    const totalQuery = applyApplicationFilters(
    ...
    const byStatus = Object.fromEntries(APPLICATION_STATUSES.map((item) => [item, 0]));
    APPLICATION_STATUSES.forEach((item, index) => {
      byStatus[item] = statusResults[index]?.count || 0;
    });

    const safeData = (data || []).map((row) => ({
      ...row,
      notes: typeof row.notes === 'string' ? row.notes.slice(0, 500) : row.notes,
    }));
    ...
    return NextResponse.json(
      { data: null, error: { code: 'ADMIN_APPLICATIONS_QUERY_FAILED', message: '获取管理员网申列表失败' } },
      { status: 500 },
    );
  }
}
