...
export default function AdminCreditsPage() {
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return query ? accounts.filter((account) => {
      const student = students.find((item) => item.user_id === account.user_id);
      return account.user_id.toLowerCase().includes(query)
        || (student?.display_name || '').toLowerCase().includes(query)
        || (student?.user_id ? `lv-${student.user_id.replace(/-/g, '').slice(0, 8)}` : '').includes(query);
    }) : accounts;
  }, [accounts, search]);
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter((student) => student.display_name.toLowerCase().includes(query) || (student.display_name || '').includes(query));
