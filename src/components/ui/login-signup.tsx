...
function AuthHeader({ title, subtitle, showLogo = false }: { title: string; subtitle?: string; showLogo?: boolean }) {
  return (
    <header className={`${showLogo ? 'mb-0' : 'mb-6'} text-center`}>
      ...
      <h2 className={`${showLogo ? 'mt-3 text-2xl text-black' : 'text-xl text-foreground'} font-semibold`}>{title}</h2>
      {subtitle && <p className={`${showLogo ? 'text-black' : 'mt-1 text-sm text-foreground'}`}>{subtitle}</p>}
    </header>
  );
}
      ...
      <AuthPage>
        <section className="w-full max-w-md">
          ...
          <CardContent className="space-y-5 px-8">
            ...
            <div className="flex items-center space-x-2"><Checkbox id="terms" checked={form.terms} onCheckedChange={(value) => update('terms', Boolean(value))} required /><label htmlFor="terms" className="text-sm text-black">{copy.agreePrefix} <Link href="#" className="text-black underline underline-offset-4">{copy.terms}</Link> {copy.and} <Link href="#" className="text-black underline underline-offset-4">{copy.conditions}</Link></label></div>
          ...
          <CardFooter className="flex justify-center border-t !py-4"><p className="text-center text-sm text-black">{copy.alreadyAccount} <BackToSignIn onClick={() => onToggleMode('password')}>{copy.signIn}</BackToSignIn></p></CardFooter>
          </Card>
        </section>
      </AuthPage>
    );
    ...
    <AuthPage>
      <section className={AUTH_PANEL_CLASS}>
        ...
        <form onSubmit={handleLoginSubmit} className="space-y-4">
          ...
          <div className="flex justify-end"><button type="button" onClick={() => onToggleMode('otp')} className="text-xs font-medium text-muted-foreground transition-colors hover:text-foreground hover:underline underline-offset-4">{copy.forgotPassword}</button></div>
