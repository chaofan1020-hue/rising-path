...
// ReactBits ships Vite asset imports. In Next.js, public URLs are the stable
// equivalent and keep GLTFLoader/useTexture inputs as strings at runtime.
...
interface LanyardRigidBody extends RapierRigidBody {
...
function Band({
  ...
  const cardMap = useMemo(() => {
    ...
    const baseImg = baseMap.image as unknown as SizedImage;
    ...
    if (frontImage && frontTex.image) drawFitted(frontTex.image as SizedImage, FRONT_UV_RECT);
    if (backImage && backTex.image) drawFitted(backTex.image as SizedImage, BACK_UV_RECT);
  ...
  return (
    <>
      ...
      <mesh ref={band}>
        <meshLineGeometry />
        <meshLineMaterial args={[{ resolution: new THREE.Vector2(1000, isMobile ? 2000 : 1000) }]} color="white" depthTest={false} resolution={isMobile ? [1000, 2000] : [1000, 1000]} useMap={1} map={bandTexture} repeat={[-4, 1]} lineWidth={lanyardWidth} />
      </mesh>
    </>
  );
}
